...
export default function RemindersPage() {
    ...
    const followUpRequestsQuery = useMemoFirebase(() => {
        ...
        // Superadmin sees all requests. Counselor sees only theirs.
        if (!isSuperAdmin) {
            q = query(q, where('counselorId', '==', user.uid));
        }

        return q;
    }, [user, userData, isSuperAdmin, firestore]);
    ...
    const isLoading = isUserLoading || isUserDataLoading || areRequestsLoading;
    ...
    return (
        <div className="space-y-8">
             <div>
                <h1 className="text-3xl font-bold font-headline">Demandes de Rappel</h1>
                <p className="text-muted-foreground">Consultez les demandes de suivi des participants après un événement live.</p>
            </div>
