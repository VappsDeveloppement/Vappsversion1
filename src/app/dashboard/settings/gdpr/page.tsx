...
export default function GdprPage() {
  ...
  const [settings, setSettings] = useState(defaultGdprSettings);
  ...
  const agencyUsersQuery = useMemoFirebase(() => {
    ...
    return query(usersCollectionRef, where('counselorIds', 'array-contains', user.uid));
  }, [user, firestore]);
  ...
  const handleMarkRequestAsClosed = async () => {
    if (!requestToProcess || !personalization?.emailSettings) {
        toast({ title: "Erreur de configuration", description: "Les paramètres d'email ne sont pas configurés.", variant: "destructive" });
        return;
    }
  ...
  const handleDeleteUserAccount = async () => {
      if (!userForRequest || !personalization?.emailSettings) {
        toast({ title: "Erreur de configuration", description: "Les paramètres d'email ne sont pas configurés ou l'utilisateur est introuvable.", variant: "destructive" });
        return;
      }
      setIsProcessing(true);
      try {
