        ...
        <Sheet>
            ...
            <SheetContent className="sm:max-w-lg w-full">
                ...
                <ScrollArea className="h-[calc(100vh-8rem)] pr-4">
                    {areRegistrationsLoading ? <Skeleton className="h-24 w-full" />
...
export default function AdminEventsPage() {
  ...
  const eventsQuery = useMemoFirebase(() => {
    ...
    // Fetch events created by the agency
    return query(collection(firestore, 'events'), where('counselorId', '==', agency.id));
  }, [agency, firestore]);
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    const eventData: Partial<Event> = {
      ...data,
      counselorId: agency.id, // Associate with the main agency
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold font-headline">Événements de la Plateforme</h1>
          <p className="text-muted-foreground">Créez et gérez les événements affichés sur la page d'accueil.</p>
        </div>
                  ...
                  <FormField control={form.control} name="imageUrl" render={() => (
                    <FormItem>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <input type="file" id="event-image-admin" className="hidden" accept="image/*" onChange={handleImageUpload} />
                        <Button type="button" variant="outline" size="sm" onClick={() => document.getElementById('event-image-admin')?.click()}>Choisir une image</Button>
                      </div>
                    </FormItem>
                  )}/>
                  <FormField control={form.control} name="isPublic" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4"><div className="space-y-0.5"><FormLabel>Événement public</FormLabel><p className="text-sm text-muted-foreground">Rendre cet événement visible sur la page d'accueil principale.</p></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl></FormItem>)} />
                </div>
              </ScrollArea>
